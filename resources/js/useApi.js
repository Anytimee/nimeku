...
export async function fetchSearchedAnime(query) {
    ...
    if (cachedData) {
        return cachedData.filter(
            (anime) =>
                anime.format !== "MANGA" && anime.status !== "NOT_YET_RELEASED"
        );
    }
    ...
    try {
        ...
        const searchedAnime = response.data.results.filter(
            (anime) =>
                anime.format !== "MANGA" && anime.status !== "NOT_YET_RELEASED"
        );
...
export async function fetchSkipTimes({
    ...
    const url = new URL(
        `${API_BASE_URL}v2/skip-times/${malId}/${episodeNumber}`
    );
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Error fetching skip times: ${response.statusText}`
            );
        }
...
export async function fetchAnimeEmbeddedEpisodes(episodeId) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Error fetching embedded episodes servers: ${response.statusText}`
            );
        }
...
export async function fetchAnimeStreamingLinks(episodeId) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Error fetching anime streaming links: ${response.statusText}`
            );
        }
